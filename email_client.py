...
class SettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.resize(450, 500)  # 增加高度以适应版本信息
        ...
        # 在版本信息后添加账户信息
        self.account_group = QGroupBox("当前账户")
        account_layout = QFormLayout()

        if parent.current_account:
            acc = parent.current_account
            account_layout.addRow("邮箱:", QLabel(acc['email']))
            account_layout.addRow("协议:", QLabel(acc['protocol']))
            account_layout.addRow("服务器:", QLabel(f"{acc['server']}:{acc['port']}"))
            account_layout.addRow("SMTP:", QLabel(f"{acc.get('smtp_server', '未设置')}:{acc.get('smtp_port', '')}"))
        else:
            account_layout.addRow(QLabel("没有活跃账户"))

        self.account_group.setLayout(account_layout)
        layout.insertWidget(3, self.account_group)  # 插入到版本信息前

        # 版本信息
        self.version_group = QGroupBox("版本信息")
        version_layout = QVBoxLayout()

        version_label = QLabel("Yanyn Email 版本: 0.14.5")
        version_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        version_label.setStyleSheet("font-weight: bold; font-size: 12pt;")

        build_date = QLabel("编写人员：深水6")
        build_date.setAlignment(Qt.AlignmentFlag.AlignCenter)

        copyright_label = QLabel("©2025 晏阳技术组,Licensed under MIT.")
        copyright_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        copyright_label.setStyleSheet("color: #666;")

        version_layout.addWidget(version_label)
        version_layout.addWidget(build_date)
...
class AddAccountDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowIcon(QIcon("icon.ico"))
        self.resize(500, 400)  # 增加高度以适应SMTP设置

        layout = QFormLayout(self)

        # 预设邮箱选择
        self.preset_combo = QComboBox()
        self.preset_combo.addItems(["晏阳邮箱", "163邮箱", "126邮箱", "QQ邮箱", "Outlook", "Gmail", "自定义"])
        ...
        # 协议选择
        self.protocol_label = QLabel("收件协议:")
        ...
        # 收件服务器地址
        self.server_label = QLabel("收件服务器:")
        self.server_edit = QLineEdit()
        layout.addRow(self.server_label, self.server_edit)

        # 收件端口
        self.port_label = QLabel("收件端口:")
        ...
        # SMTP服务器设置组
        self.smtp_group = QGroupBox("发件服务器设置 (SMTP)")
        smtp_layout = QFormLayout()

        self.smtp_server_edit = QLineEdit()
        self.smtp_port_edit = QLineEdit()
        self.smtp_ssl_check = QCheckBox("使用SSL")
    ...
    def update_preset_settings(self, preset):
        ...
        self.smtp_group.setVisible(is_custom)

        if not is_custom and preset in preset_configs:
            protocol = self.protocol_combo.currentText().lower()
            config = preset_configs[preset]

            # 设置收件服务器
            self.server_edit.setText(config[protocol]["server"])
            self.port_edit.setText(str(config[protocol]["port"]))
            self.ssl_check.setChecked(config[protocol]["ssl"])

            # 设置发件服务器
            self.smtp_server_edit.setText(config["smtp"]["server"])
            self.smtp_port_edit.setText(str(config["smtp"]["port"]))
            self.smtp_ssl_check.setChecked(config["smtp"]["ssl"])
...
class EmailClient(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Yanyn Email 0.14.5")
        self.setWindowIcon(QIcon("icon.ico"))
        ...
        with QMutexLocker(self.email_mutex):
            for raw_email in self.emails:
                try:
                    ...
                    display_from = from_name if from_name else from_addr
                    ...
                    # 创建列表项
                    item = QTreeWidgetItem([display_from, subject, date])
    ...
    def show_email(self):
        ...
        for attachment in self.current_attachments:
            self.attachment_list.addItem(attachment['filename'])

        # 连接附件列表的双击信号
        self.attachment_list.itemDoubleClicked.connect(self.handle_attachment_click)
    ...
    def handle_attachment_click(self, item):
        ...
        filename = item.text()
        attachment = next((a for a in self.current_attachments if a['filename'] == filename), None)
        if not attachment:
            return
        ...
        reply = QMessageBox.question(
            self, "附件操作",
            f"如何处理附件 '{filename}'?",
            QMessageBox.StandardButton.Save | QMessageBox.StandardButton.Open | QMessageBox.StandardButton.Cancel,
        ...
        if reply == QMessageBox.StandardButton.Save:
            save_path, _ = QFileDialog.getSaveFileName(
                self, "保存附件",
                filename,
                f"所有文件 (*.*);;{self.get_file_filter(attachment['content_type'])}"
            )
            if not save_path:
                return
        else:  # Open
        ...
        try:
            ...
            QMessageBox.information(self, "成功", f"附件 '{filename}' 已保存到 {save_path}")
        except Exception as e:
            logging.error(f"处理附件失败: {str(e)}")
            QMessageBox.critical(self, "错误", f"处理附件失败:\n{str(e)}")

    def get_file_filter(self, content_type):
        """根据内容类型获取文件过滤器"""
        type_map = {
            ...
            'application/x-rar-compressed': 'RAR 压缩文件 (*.rar)'
        }
        return type_map.get(content_type, '所有文件 (*.*)')
